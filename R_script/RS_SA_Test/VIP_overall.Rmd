...
* **Result by Simple Model**

```{r, echo=FALSE,fig.width=10,fig.height=5, cache=TRUE}
count(ID_OD,"Predicted")
ggplot(ID_OD,aes(Hour,fill=as.factor(Predicted)))+geom_bar(binwidth=1) + xlim(c(0,25)) 
```
...
```{r, echo=FALSE,fig.width=10,fig.height=5, cache=TRUE}
count(ID_OD,"ML")
ggplot(ID_OD,aes(Hour,fill=as.factor(ML)))+geom_bar(binwidth=1) + xlim(c(0,25)) 
```
...
```{r,echo=FALSE,fig.width=15,fig.height=12, cache=TRUE}
ggplot()+geom_segment(data=na.omit(ID_OD),aes(x=Elng, y=Elat,xend=Slng, yend=Slat), col="darkgreen",alpha=.2,lwd=1)+
  geom_point(data=ID_OD,aes(Slng,Slat),col="darkblue",alpha=.2)+
  facet_wrap(~Nom,ncol=2)+
   geom_point(data=gares_geo[gares_geo$Societe != 5,],aes(Lng,Lat),alpha=.2)
```
...
ggplot(ID_OD,aes(Hour,fill=as.factor(Predicted)))+geom_bar(binwidth=1)+facet_wrap(~Nom, ncol=2)
...
ggplot(ID_OD[ID_OD$Nom==ID_ref$Nom[i],],aes(Hour,fill=as.factor(Predicted)))+geom_bar(binwidth=1) + xlim(c(0,25)) 
```
...
ggplot(temp_AR[temp_AR$Year==2014 & temp_AR$DOW %in% jrOuvert,],aes(WOY,fill=as.factor(Predicted)))+geom_bar(binwidth=1) + ggtitle(temp_title)

```

**FF: AR-DT**

```{r,echo=FALSE,fig.width=10,fig.height=6, cache=TRUE}

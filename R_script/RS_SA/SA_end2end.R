...
trx <- trx.ready %>% filter(Date >= train.start & Date < test.end)
...
##########
Ind <- rbind(ind.model.0, ind.model.1)
...
result <- rbind(result.model.decade.0, result.model.decade.1)
...
### other action
temp1 <- ID.ref %>% transmute(ID = as.character(Nom), N.badge = paste0(Ste,N.badge), EVA)
temp <- inner_join(temp1, result.final) %>% arrange (ID, desc(noPsg), Tmin)
# output: csv file
write.table(temp, file="result.treated.csv", sep = ";", row.names = F, quote = F)

t <- trx %>% select(ID, TimeSor, DOW, Date, OD)
write.table(t, file="input_format.csv", sep = ";", row.names = F, quote = F)
